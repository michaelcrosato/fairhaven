...
void AUEGT2HUD::DrawCrosshair(float CentreX, float CentreY, bool bHasFocus)
{
	...
	const FLinearColor Colour = bHasFocus ? Accent : FLinearColor(1, 1, 1, 0.55f);
...
void AUEGT2HUD::DrawPrompt(float CentreX, float CentreY)
{
	...
	DrawRect(Shade, X - 12.0f, Y - 6.0f, Width + 24.0f, Height + 12.0f);
	DrawText(Text, Ink, X, Y, Font, 1.0f, false);
}
...
void AUEGT2HUD::DrawMessage(float CentreX, float ScreenHeight)
{
	...
	DrawRect(FLinearColor(Shade.R, Shade.G, Shade.B, Shade.A * Alpha),
		X - 14.0f, Y - 7.0f, Width + 28.0f, Height + 14.0f);
	DrawText(Text, FLinearColor(Ink.R, Ink.G, Ink.B, Alpha), X, Y, Font, 1.0f, false);
}
...
void AUEGT2HUD::DrawDiagnostics(AUEGT2Character* Explorer)
{
	...
	{
		DrawText(Lines[Index], Index == 0 ? Accent : Muted, 28.0f, Y, Font, 1.0f, false);
		Y += LineHeight;
	}
}
